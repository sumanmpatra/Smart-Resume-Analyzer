...
def generate_ml_suggestions(resume_analysis, resume_skills, jd_skills, missing_skills, similarity_score):
    ...
    missing_sections = [section for section, present in resume_analysis['sections'].items() if not present]
    if missing_sections:
        suggestions.append({
...
def calculate_resume_score(resume_analysis, similarity_score, matched_skills, missing_skills):
    ...
    content_score += min(resume_analysis['quantifiable_achievements'] / 5, 1) * 5
    score += content_score
...
if resume_file and jd_file:
    with st.spinner("Analyzing your resume using Advanced ML & NLP..."):
        ...
        jd_text = (extract_text_from_pdf(jd_file) if jd_file.name.endswith(".pdf") 
...
if resume_analysis:
    overall_score = calculate_resume_score(resume_analysis, score, matched_skills, missing_skills)
    st.markdown("### 📊 Overall Resume Quality Score")
    st.progress(overall_score / 100)
    
    if overall_score >= 80:
        st.success(f"Excellent! Your resume scores **{overall_score:.1f}/100**")
    elif overall_score >= 60:
        st.warning(f"Good! Your resume scores **{overall_score:.1f}/100** - Room for improvement")
    else:
        st.error(f"Your resume scores **{overall_score:.1f}/100** - Needs significant improvement")

# --- SKILLS ANALYSIS SECTION ---
st.markdown("### 🧠 Skills Analysis")

with st.expander("🔍 Detailed Skills Report", expanded=True):
    st.markdown(f"✅ **Matched Skills:** `{', '.join(sorted(matched_skills)) or 'None'}`")
...
if matched_skills or missing_skills:
    ...
    with col2:
        ...
        bars = ax.bar(['Matched Skills', 'Missing Skills'], counts, color=colors)
        ax.set_ylabel('Count')
    ...
    with col3:
        ...
        if not labels:
            ...
            matched_vals = [1 if skill in matched_skills else 0 for skill in jd_skills]
            matched_vals += matched_vals[:1]

            fig, ax = plt.subplots(figsize=(4, 4), subplot_kw=dict(polar=True))
            ax.plot(angles, matched_vals, color="#00C49F", linewidth=2, label='Matched Skills')
...
if resume_file and resume_analysis:
    suggestions = generate_ml_suggestions(resume_analysis, resume_skills, jd_skills, missing_skills, score)
    ...
    with st.expander("📊 Detailed Resume Analytics", expanded=False):
        ...
        with col1:
            ...
            st.write(f"• Avg Sentence Length: {resume_analysis['avg_sentence_length']:.1f}")
            st.write(f"• Action Verbs Used: {resume_analysis['action_verb_count']}")
            st.write(f"• Quantified Achievements: {resume_analysis['quantifiable_achievements']}")
        ...
        with col2:
            ...
            st.write(f"• Readability Score: {resume_analysis['readability_score']:.1f}/100")
